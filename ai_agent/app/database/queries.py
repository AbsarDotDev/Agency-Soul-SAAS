...
class DatabaseQueries:
        ...
        try:
            ...
            if employee_id:
                query = text("""
                    ...
                    LEFT JOIN departments d ON e.department_id = d.id
                    LEFT JOIN designations ds ON e.designation_id = ds.id
                    WHERE e.company_id = :company_id AND e.id = :employee_id
                """)
            ...
            if customer_id:
                query = text("""
                    SELECT c.id, c.name, c.email, c.contact, c.billing_name, c.billing_country,
                           ...
                           c.shipping_phone, c.shipping_address, c.created_by
                    FROM customers c
                    WHERE c.company_id = :company_id AND c.id = :customer_id
                """)
    ...
    @staticmethod
    async def get_expense_breakdown(company_id: int, session: Session) -> List[Dict[str, Any]]:
        """Get expense breakdown by category for a company."""
        try:
            query = text("""
            ...
            # Convert to list of dictionaries
            expense_breakdown = [{"category": row[0], "total_amount": float(row[1])} for row in result]
            
            return expense_breakdown
            
        except SQLAlchemyError as e:
            logger.error(f"Database error when retrieving expense breakdown: {str(e)}")
    ...
    async def insert_revenue(company_id: int, user_id: int, parameters: Dict[str, Any], session: Session) -> Optional[int]:
        ...
        try:
            ...
            customer_check_query = text("""
                SELECT id FROM customers WHERE id = :customer_id AND created_by IN (SELECT id FROM users WHERE created_by = :company_id)
            """)
            ...
            account_check_query = text("""
                SELECT id FROM bank_accounts WHERE id = :account_id AND created_by IN (SELECT id FROM users WHERE created_by = :company_id)
            """)
            ...
            category_check_query = text("""
                SELECT id FROM product_service_categories WHERE id = :category_id AND created_by IN (SELECT id FROM users WHERE created_by = :company_id)
            """)
